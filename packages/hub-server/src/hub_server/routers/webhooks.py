...
@router.get("/{job_key}/callbacks/{callback_id}/deliveries")
def list_callback_deliveries(
    job_key: str,
    callback_id: int,
    session: Annotated[Session, Depends(get_session)],
) -> dict[str, object]:
    """List the delivery attempts of one callback, newest first (max 100)."""
    job = session.query(Job).filter(Job.job_key == job_key).one_or_none()
    if job is None:
        raise HubError(code="JOB_NOT_FOUND", message="任务不存在", status_code=404)
    registered = session.get(JobCallback, callback_id)
    ...
    deliveries = (
        ...
        .filter(WebhookDelivery.callback_id == callback_id)
        .order_by(WebhookDelivery.attempted_at.desc(), WebhookDelivery.id.desc())
        .limit(100)
        .all()
    )
    return {"items": [_delivery_response(delivery) for delivery in deliveries]}
...
def _delivery_response(delivery: WebhookDelivery) -> dict[str, object]:
    """Serialize one delivery attempt row."""
    return {
        "id": delivery.id,
        "attempted_at": _utc_iso(delivery.attempted_at),
