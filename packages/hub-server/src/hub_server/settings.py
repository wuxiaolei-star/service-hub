...
from python_hub_contracts import normalize_public_key
...
class PluginSignatureSettings(StrictSettingsModel):
    """Optional artifact-signature policy for uploaded plugin packages (B9/G9).

    Keys are Ed25519 public keys in base64 (raw 32 bytes), generated by
    ...
    - ``require_signed: true``: 未附带签名的包也 422 拒收(强制的最小公钥数为 1,
      启动期即拒绝矛盾配置, 避免静默放行);
    - 支持公钥轮换: 列表内任一公钥验过即通过, 轮换时先加新钥再删旧钥.
    """
    ...
    @classmethod
    def _validate_public_keys(cls, value: list[str]) -> list[str]:
        """Reject malformed keys at load time instead of at first verification."""
        if not value:
            return value
        return [normalize_public_key(entry) for entry in value]

    @model_validator(mode="after")
    def _require_signed_needs_at_least_one_key(self) -> Self:
        """Enforcing signatures with zero trusted keys would reject every upload."""
        if self.require_signed and not self.public_keys:
            raise ValueError(
                "plugins.signature.require_signed=true requires at least one "
                "plugins.signature.public_keys entry"
            )
        return self


class PluginsSettings(StrictSettingsModel):
    """Kernel feature plugin switches (V4.0) and package-signature policy (B9)."""

    enabled: list[str] | None = None
    options: dict[str, dict[str, object]] = Field(default_factory=dict)
    signature: PluginSignatureSettings = Field(default_factory=PluginSignatureSettings)
